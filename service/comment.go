...
func Getcomment(model_ int,target_id int,page int,size int)([]model.Responses1,[]model.Responses2) {
	 array1:=make([]model.Responses1,0)
	 array2:=make([]model.Responses2,0)
	if model_ == 1 {
		sql := `select comment_id,post_id,publish_time,content,pictures,user_id,avatar,nickname,praise_count from responses1 where post_id=?`
		...
		sql := `select comment_id,post_id,publish_time,content,reply_user_id,avatar,reply_nickname,praise_count from responses2 where target_id=?`
		rows, err := dao.QueryMore(sql, target_id)
		defer rows.Close()
		if err != nil {
			fmt.Println(err)
...
func Publishcomment(model int,post_id int,target_id int,content string,photo string,nickname string,user_id string,avatar string,publish_time string,praise_count int)int64{
	if model==1{
		sqlstr:=`insert into responses1 (post_id,content,pictures,nickname,user_id,avatar,publish_time,praise_count)values(?,?,?,?,?,?,?,?)`
		return dao.Insertdb(sqlstr,target_id,content,photo,nickname,user_id,avatar,publish_time,praise_count)
	}else{
		sqlstr:=`insert into responses2 (post_id,target_id,content,reply_nickname,reply_user_id,avatar,publish_time,praise_count)values(?,?,?,?,?,?,?,?)`
...
func Getcomment_id(content string,model_ int)int{
	sqlstr1:=`select comment_id from responses1 where content=?`
	sqlstr2:=`select comment_id from responses2 where content=?`
	comment_id:=0
	if model_==1{
